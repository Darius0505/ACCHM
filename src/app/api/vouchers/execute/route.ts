...
export async function POST(request: NextRequest) {
    try {
        ...
        for (const step of workflows) {
            ...
            } else if (step.taskType === 'INSERT_PAYMENT') {
                if (action === 'SAVE') {
                    if (data.id) {
                        const { updateCashPayment } = await import('@/services/cashPayment.service');
                        resultData = await updateCashPayment(data.id, {
                            ...data,
                            date: data.date ? new Date(data.date) : new Date(),
                            amount: data.amount ? parseFloat(data.amount.toString()) : 0,
                            status: data.status || 'POSTED'
                        ...
                        const { createCashPayment } = await import('@/services/cashPayment.service');
                        resultData = await createCashPayment({
                            companyId: company.id,
                            ...data,
                            date: data.date ? new Date(data.date) : new Date(),
                            amount: data.amount ? parseFloat(data.amount.toString()) : 0,
                    ...
                    if (data.id) {
                        const { deleteCashPayment } = await import('@/services/cashPayment.service');
                        await deleteCashPayment(data.id);
                        resultData = { id: data.id, deleted: true };
                    } else {
                        throw new Error('Cannot delete without ID');
                    }
                    ...
                    if (data.id) {
                        const { getCashPayment } = await import('@/services/cashPayment.service');
                        const { previewEntryNumber } = await import('@/services/numberSequence.service');

                        const original = await getCashPayment(data.id);
                        if (!original) throw new Error('Bản gốc không tồn tại');
                        ...
                        resultData = {
                            ...
                            attachedFiles: [],
                            journalEntryId: null,
                            details: original.details?.map((d: any) => ({ ...d, id: crypto.randomUUID(), cashPaymentId: undefined })) || []
                        };
                    } else {
                        throw new Error('Cannot copy without ID');
                    }
                }
            } else if (step.taskType === 'INSERT_BANK_RECEIPT' || step.taskType === 'INSERT_BANK_PAYMENT') {
                if (action === 'SAVE') {
                    if (data.id) {
                        const { updateBankTransaction } = await import('@/services/bankTransaction.service');
                        resultData = await updateBankTransaction(data.id, {
                            ...data,
                            date: data.date ? new Date(data.date) : new Date(),
                            amount: data.amount ? parseFloat(data.amount.toString()) : 0,
                            status: data.status || 'POSTED'
                        ...
                        const { createBankTransaction } = await import('@/services/bankTransaction.service');
                        resultData = await createBankTransaction({
                            companyId: company.id,
                            ...data,
                            date: data.date ? new Date(data.date) : new Date(),
                            amount: data.amount ? parseFloat(data.amount.toString()) : 0,
                    ...
                    if (data.id) {
                        const { deleteBankTransaction } = await import('@/services/bankTransaction.service');
                        await deleteBankTransaction(data.id);
                        resultData = { id: data.id, deleted: true };
                    } else {
                        throw new Error('Cannot delete without ID');
                    }
                    ...
                    if (data.id) {
                        const { getBankTransaction } = await import('@/services/bankTransaction.service');
                        const { previewEntryNumber } = await import('@/services/numberSequence.service');

                        const original = await getBankTransaction(data.id);
                        if (!original) throw new Error('Bản gốc không tồn tại');
                        ...
                        resultData = {
                            ...
                            attachedFiles: [],
                            journalEntryId: null,
                            details: original.details?.map((d: any) => ({ ...d, id: crypto.randomUUID(), bankTransactionId: undefined })) || []
                        };
                    } else {
                        throw new Error('Cannot copy without ID');
                    }
