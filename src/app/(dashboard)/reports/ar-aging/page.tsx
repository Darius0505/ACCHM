...
export default function ARAgingPage() {
    ...
    const [data, setData] = useState<{ summary: AgingSummary; details: AgingRow[] } | null>(null);

    const fetchData = async () => {
        setLoading(true);
        try {
            const res = await fetch(`/api/reports/ar-aging?date=${asOfDate}`);
    ...
    return (
        <div className="p-6">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-bold">Báo Cáo Tuổi Nợ Phải Thu</h1>
            ...
            {loading ? (
                ...
                <div className="bg-white rounded shadow p-8 text-center text-gray-500">
                    Không có công nợ phải thu tại thời điểm này.
                </div>
            ) : (
                        ...
                        <tbody>
                            {data.details.map((row, idx) => (
                                <tr key={idx} className="border-b hover:bg-gray-50">
                                    <td className="p-4">
                                        <span className="font-medium">{row.code}</span> - {row.partnerName}
                                    </td>
            ...
            <div className="mt-6 text-sm text-gray-500">
                <p><strong>Chú thích:</strong> Tuổi nợ tính theo số ngày kể từ ngày đến hạn thanh toán (Due Date).</p>
            </div>
        </div>
    );
}
