...
const PortalDropdown = ({ position, accounts, onSelect, searchTerm, selectedIndex }: any) => {
    // Portal content
    const content = (
        <div
                ...
                <Command.List>
                    ...
                    {accounts.map((acc: AccountOption) => (
                        <Command.Item
                            key={acc.code}
                            value={acc.code}
                            onSelect={() => onSelect(acc.code)}
                        // We rely on Command's internal selection state which is driven by keyboard
                        >
...
const AccountGridEditor = forwardRef((params: any, ref) => {
    ...
    const accounts: AccountOption[] = params.accounts || params.colDef.cellEditorParams?.accounts || [];
    ...
    // Filter accounts here for the separate list
    const filteredAccounts = accounts.filter(acc =>
        acc.code.toLowerCase().includes(value.toLowerCase()) ||
    ...
    const handleKeyDown = (e: React.KeyboardEvent) => {
        if (e.key === 'ArrowDown') {
            ...
            const selected = filteredAccounts[highlightedIndex];
            // If selected exists, use it. Otherwise use what user typed.
            const codeToSave = selected ? selected.code : value;
                    ...
                    filteredAccounts.map((acc, idx) => (
                        <div
                            ...
                            onMouseDown={(e) => {
                                ...
                                if (params.node && params.column) {
                                    params.node.setDataValue(params.column.colId, acc.code);
                                }
                                params.stopEditing();
                            }}
...
AccountGridEditor.displayName = 'AccountGridEditor';
export default AccountGridEditor;
