 ...
 * Re-designed using Hybrid Core Architecture and 2-column Layout
 */
...
import { CashPaymentReport } from '@/components/reports/CashPaymentReport';
import { CashPaymentReport2Lien } from '@/components/reports/CashPaymentReport2Lien';
...
export default function CashPaymentForm({ id, onSuccess, onCancel }: CashPaymentFormProps) {
    ...
    const fetchNextNumber = useCallback(async () => {
        try {
            const res = await fetch('/api/cash-payments/next-number');
            if (res.ok) {
                const data = await res.json();
    ...
    const fetchVoucherApi = useCallback(async (paymentId: string) => {
        const res = await fetch(`/api/cash-payments/${paymentId}`);
        ...
        return {
            id: full.id,
            paymentNumber: full.paymentNumber,
            date: dateStr,
            postedDate: postedDateStr,
            payeeName: full.payeeName || full.partner?.name || '',
            payeeAddress: full.payeeAddress || '',
            subjectType: full.partner?.type || 'VENDOR',
            ...
            details: full.details?.length > 0
                ...
                : [{
                    ...
                    debitAccount: full.debitAccountId || '331',
                    creditAccount: full.creditAccountId || '1111'
                }],
    ...
    const executeApi = useCallback(async ({ journalCode, action, data }: { journalCode: string, action: string, data: any }) => {
        ...
        if (action === 'SAVE') {
            ...
            const payload = {
                ...
                debitAccountId: data.details[0]?.debitAccount || '331',
                creditAccountId: data.details[0]?.creditAccount || '1111',
            ...
            if (!res.ok) {
                ...
                if (isNewMode && err.error?.includes('Unique constraint failed')) {
                    throw new Error(`Số phiếu ${data.paymentNumber} đã tồn tại. Vui lòng lấy số mới và lưu lại.`);
                }
    ...
    useEffect(() => {
        if (isNewMode) {
            ...
            fetchNextNumber().then(num => {
                if (num) setForm(f => ({ ...f, paymentNumber: num }));
            });
        } else if (id) {
            loadVoucher(id);
        }
    ...
    const handleSaveWrapper = () => {
        ...
        if (!form.payeeName) {
            toast({ title: "Thiếu thông tin", description: "Vui lòng nhập người nhận tiền", variant: "destructive" });
            return;
        }
        if (totalAmount <= 0) {
    ...
    const handleDeleteWrapper = () => {
        const msg = isPosted
            ? `Phiếu ${form.paymentNumber} đã ghi sổ.\n\nBạn có muốn XOÁ phiếu này không?\n(Bút toán liên quan sẽ bị huỷ tự động)`
            : `Bạn có chắc muốn xoá phiếu ${form.paymentNumber}?`;
        if (confirm(msg)) {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%', backgroundColor: 'var(--background)', color: 'var(--text-primary)' }}>
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        <div style={{
                                        ...
                                        <ComboboxSelect
                                            ...
                                            onChange={(val) => {
                                                ...
                                                setForm(f => ({
                                                    ...
                                                    payeeName: subject?.label || f.payeeName,
                            ...
                            <div style={rowStyle}>
                                <label style={{ ...labelStyle, minWidth: '90px' }}><span style={{ color: 'red' }}>*</span> Người nhận</label>
                            ...
                            <div style={rowStyle}>
                                ...
                                <input value={form.payeeAddress} onChange={e => setForm(f => ({ ...f, payeeAddress: e.target.value }))} style={inputStyle} placeholder="Địa chỉ người nhận tiền..." />
                            </div>
                        ...
                        <div style={{
                            ...
                            <div style={rowStyle}>
                                ...
                                <input value={form.paymentNumber} onChange={e => setForm(f => ({ ...f, paymentNumber: e.target.value }))} style={{ ...inputStyle, fontSize: '11px', fontWeight: 'bold', color: 'var(--primary)', textAlign: 'left' }} />
                            </div>
                            <div style={rowStyle}>
            ...
            <PrintOptionsModal
                ...
                onConfirm={(options: PrintOptions) => {
                    ...
                    import('qrcode').then(QRCode => {
                        const qrText = `ACCHM-PC|${form.paymentNumber}|${form.date}|${totalAmount}`;
            ...
            {company && (
                <ReportViewer
                    ...
                    fileName={`Phieu_Chi_${form.paymentNumber}`}
                    document={
                        printTemplate === '2LIEN' ? (
                            <CashPaymentReport2Lien
                                company={company}
                                signatureNames={{
                                    'Giám đốc': company.directorName || '',
                                    'Kế toán trưởng': company.chiefAccountantName || '',
                                    'Người nhận tiền': form.payeeName || '',
                                }}
